...
const UserForm: React.FC<UserFormProps> = ({ userId, onSave }) => {
  ...
  return (
    <main className="w-screen h-screen flex flex-col items-center justify-center p-20 bg-primarydark bg-[url('../../assets/topography.svg')] overflow-auto">
      <form onSubmit={handleSubmit} className="min-w-[350px] w-[50%] max-w-[500px] p-10 rounded-2xl border border-black flex flex-col gap-4 justify-center items-end bg-secondarylight bg-opacity-10 backdrop-filter backdrop-blur-sm">
        ...
        <button type="submit" className="w-20 h-10 bg-secondarylight bg-opacity-50 rounded-md text-secondarylight font-bold transition-all hover:text-white hover:bg-opacity-75">{userId ? 'Update' : 'Create'}</button>
      </form>
    </main>
  );
};
